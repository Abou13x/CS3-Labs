...
Value& hashmap<Key, Value, Compare, Hash>::operator[] (const Key& x) {
    ...
    //will return flase if element exist and insert new element if it does not exist
    auto pairPointer = insert(make_pair(x, Value()));
    return pairPointer.first->second;
...
void hashmap<Key, Value, Compare, Hash>::rehash(size_t n) {
    ...
    //setting the number of buckets in the container
    hash_ = DefaultHash<Key>(n);

    //moving the old elements from the container to a temp container
    vector<Element> temp;
    ...
    elems_ = vector<list<Element>>(hash_.numBuckets());
    //rearranging container elements
    for (auto& item : temp) {
